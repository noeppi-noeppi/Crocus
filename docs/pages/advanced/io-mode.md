...
Usually, when you invoke Crocus, it performs an incremental synchronisation of all calendars in the system
 configuration.
...
| **Option**                        | **Description**                                                                  |
+===================================+==================================================================================+
| <span style="white-space:nowrap"> | Reads the given *file* as a json file in Crocus own event json format when       |
| `--load <file>`</span>            | aggregating events.                                                              |
+-----------------------------------+----------------------------------------------------------------------------------+
| <span style="white-space:nowrap"> | Dumps all events into the given *file* in Crocus event json format.              |
| `--dump <file>`</span>            |                                                                                  |
+-----------------------------------+----------------------------------------------------------------------------------+
...
| `--sink <name>`</span>            | configuration. This option causes a non-incremental synchronisation.             |
+-----------------------------------+----------------------------------------------------------------------------------+
| <span style="white-space:nowrap"> | Pushes all events into the calendar of the given *name* in the system            |
| `--isink <name>`</span>           | configuration. This option causes an incremental synchronisation.                |
+-----------------------------------+----------------------------------------------------------------------------------+
| <span style="white-space:nowrap"> | Adds a filter to the list of filters to run on all aggregated events. The format |
| `--filter <filter>`</span>        | of such a *filter specification* is explained below.                             |
+-----------------------------------+----------------------------------------------------------------------------------+
...
- `simple=` is another example using the *simple* filter from the [Filter Plugin](../plugins/filter.md).
...
Note that filter definitions often contain special characters that need to be quoted in a shell.

## Examples

Here are some examples of IO mode.
