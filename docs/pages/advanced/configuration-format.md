...
As with the `calendars` block, there are two options for adding an event source.
Event sources can be added like this:
```groovy
sources {
...
- The `into` function takes the name of a calendar as an argument and specifies that events from the event source shall
   be placed in that calendar.
  Example:
  ```groovy
...
- `globals.http` allows configuring an HTTP resource.
  ...
  - `noRedirects()` disables following HTTP redirects.
...
Crocus provides a whole range of automatic type conversions.
...
For an overview of the exact type conversions performed, see the
