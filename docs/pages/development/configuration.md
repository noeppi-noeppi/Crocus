...
A sample delegate class can be found below:
```java
...
As explained in the [configuration reference](../advanced/configuration-format.md#automatic-type-conversions), Crocus
 automatically converts values to required types in most cases.
This section gives an overview of the available conversions.
...
- `String`, `Date`, and `TemporalAccessor` are converted to `Instant`, `ZonedDateTime`, `OffsetDateTime`,
  ...
  If the resulting type has a time component and the input representation does not, the time component will be set to
   midnight.
  If the resulting type has a time zone and the input representation does not, the time zone will be set to the system
   time zone.
- `String` is converted to `ZoneId` by the time zone name.
- `String` is converted to `TemporalAmount`, `Period`, or `Duration`.
  The string must have a format like `12 days + 3 hours - 1 second`.
- `String` is converted to `DayOfWeek`.
  The string can use the three-letter abbreviation (e.g. `tue`) or the full name of the day (e.g. `thursday`).
- `String` is converted to `Month`.
  The string can use the three-letter abbreviation (e.g. `sep`) or the full name of the month (e.g. `september`).
...
- `URL`, `URI`, `Path`, `File`, `String`, and `byte[]` are converted to `Resource`.
  ...
  Strings can only be converted to resources if they are valid URIs or paths.
